    ...
    <div className="flex-1 overflow-auto p-4 md:p-8 bg-slate-100 flex justify-center">
      <div className="w-full max-w-3xl bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden flex flex-col">
        <div className="bg-brand-600 p-6 text-white shrink-0">
          ...
          <p className="opacity-80 text-base mt-1">Configure as regras e participantes para: <strong>{project.name}</strong></p>
        </div>
        ...
        <div className="p-6 space-y-8 flex-1 overflow-auto">
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-semibold text-slate-700 border-b pb-2">Regras Gerais</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <label className="flex items-center gap-3 p-4 border rounded-lg cursor-pointer hover:bg-slate-50 transition-colors bg-white">
                ...
                <div>
                  ...
                  <span className="text-sm text-slate-500">Ida e volta (todos contra todos 2x)</span>
                </div>
              </label>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end border-b pb-2">
              <h3 className="text-xl font-semibold text-slate-700">Participantes <span className="text-base font-normal text-slate-500">({teams.length})</span></h3>
              ...
              {inputMode === 'single' ? (
                ...
                <div className="flex flex-col gap-2">
                  <textarea
                    ...
                    rows={5}
                    className="w-full bg-white text-slate-900 border border-slate-300 rounded p-3 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 text-base"
                  />
                  <div className="flex justify-end">
                    <button
                      onClick={addBulkTeams}
                      disabled={!bulkTeamsInput.trim()}
                      className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded font-medium flex items-center gap-2 disabled:opacity-50 transition-colors text-base"
                    >
            ...
            <div className="border rounded-lg divide-y divide-slate-100 bg-white shadow-sm">
              <div className="p-2 bg-slate-50 border-b flex justify-between items-center">
                <span className="text-sm font-bold text-slate-500 uppercase tracking-wider">Times Adicionados</span>
                {teams.length > 0 && (
                  <button onClick={clearAllTeams} className="text-sm text-red-500 hover:underline">Limpar tudo</button>
                )}
              </div>
          ...
          <div className="pt-4 border-t flex justify-end shrink-0">
            <button
              ...
              className="bg-green-600 hover:bg-green-700 text-white text-xl px-8 py-3 rounded-lg font-bold shadow-md hover:shadow-lg transform transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
